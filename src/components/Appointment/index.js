...
export default function Appointment(props) {
  ...
  function deleteInterview (id) {
    ...
    transition(DELETE);
    props.cancelInterview(id).then(()=>{transition(DELETE);transition(EMPTY)}).catch(error => transition(ERROR_DELETE,true));
  ...
  function save(name, interviewer) {
    ...
    props.bookInterview(id,interview).then(()=>{
      ...
      .catch((error) => {transition(ERROR_SAVE, true); console.log(error)});
  }
  ...
  return (
    <article data-testid="appointment" className="appointment">
      ...
      {mode === EDIT && <Form student={interview.student} interviewer={interview.interviewer} onSave={save} onCancel={back} interviewers={interviewers} />}
